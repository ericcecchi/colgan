...
function wp_embed_handler_html5_audio( $matches, $attr, $url, $rawattr ) {
	...
	if (preg_match('#^http://.+\.mp3$#i', $url) && preg_match('/Firefox/', $_SERVER["HTTP_USER_AGENT"])) {
		...
		$embed = sprintf(
				'<embed type="application/x-shockwave-flash" flashvars="audioUrl=%1$s" src="http://www.google.com/reader/ui/3523697345-audio-player.swf" width="400" height="27" quality="best"></embed>',
				esc_attr($matches[0])
				);
		...
		$embed = sprintf(
				'<audio controls preload><source src="%1$s" /><embed type="application/x-shockwave-flash" flashvars="audioUrl=%1$s" src="http://www.google.com/reader/ui/3523697345-audio-player.swf" width="400" height="27" quality="best"></embed></audio>',
				esc_attr($matches[0])
				);
	}
